...
// Заполнить филиал посещение аттракциона.
Процедура ЗаполнитьФилиалПосещениеАттракциона(ПараметрыОбработки) Экспорт
	
	ПараметрыОбработки.ОбработкаЗавершена = Ложь;
	
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ
	|	ПосещениеАттракциона.Ссылка
	|ИЗ
	|	Документ.ПосещениеАттракциона КАК ПосещениеАттракциона
	|ГДЕ
	|	ПосещениеАттракциона.Проведен = ИСТИНА
	|	И ПосещениеАттракциона.Филиал = ЗНАЧЕНИЕ(Справочник.Филиалы.ПустаяСсылка)
	|	И ПосещениеАттракциона.Основание <> ЗНАЧЕНИЕ(Документ.ПродажаБилета.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПосещениеАттракциона.Дата";
